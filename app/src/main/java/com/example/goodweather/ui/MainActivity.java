...
import com.example.goodweather.ui.adapter.HourlyAdapter;
...
import com.example.goodweather.utils.GlideUtils;
...
public class MainActivity extends NetworkActivity<ActivityMainBinding> implements LocationCallback, CityDialog.SelectedCityCallback {
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if (itemId == R.id.item_switching_cities) {        //切换城市
            if (cityDialog != null) cityDialog.show();
        } else if (itemId == R.id.item_relocation) {              //重新定位
            startLocation();//点击重新定位item时，再次定位一下。
        } else if (itemId == R.id.item_bing) {                    //是否使用必应壁纸
